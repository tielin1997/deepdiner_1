...
TEngine 事件模块是一个**高效且零 GC**的事件系统，支持指定事件 ID（int/string）进行监听和分发。通过事件来驱动模块，如战斗的角色身上的事件流、UI 和网络以及 Model 的数据流，开发中的绝大部分情况都可以通过事件来进行驱动。

配合 UI 模块或拓展的战斗模块可以实现 **MVE（Model - View - Event）事件驱动架构**。

### ✨ 核心优势

1. **⚡ 零 GC 设计**
   - 使用 int 类型事件 ID，避免字符串哈希计算
   - 优化的数据结构，减少内存分配
   - 适合高频事件场景

2. **🔧 灵活的事件类型**
   - 支持 `int` 类型事件 ID（推荐，性能最佳）
   - 支持 `string` 类型事件 ID（方便调试）
   - 使用 `RuntimeId.ToRuntimeId` 方法定义事件 ID，达到最佳性能

3. **🎯 生命周期自动管理**
   - UI 模块的事件和 UI 生命周期存在绑定
   - 销毁 UI 时可以自动移除 UI 所监听的事件
   - 开发过程中只需要关心添加事件，避免了关闭 UI 但没有移除事件监听的问题
...
```csharp
// 方式1：使用 RuntimeId.ToRuntimeId（推荐，性能最佳）
public static readonly int EventPlayerHpChange = RuntimeId.ToRuntimeId("Player.HpChange");
public static readonly int EventPlayerLevelUp = RuntimeId.ToRuntimeId("Player.LevelUp");

// 方式2：直接使用字符串（方便调试，但性能略低）
...
当服务器发来减少 Hp 的消息包时，我们可以：

1. 在收到消息包时发送一个事件流
2. 在玩家头顶的 HP 进度条组件/左上角 Hp 的 UI 血条组件添加监听事件
3. 各个模块负责自己监听后的逻辑
...
public static class PlayerEventDefined
{
    ...
    public static readonly int OnLevelUp = RuntimeId.ToRuntimeId("Player.LevelUp");
}
...
// 移除所有 UI 事件（通常在 OnDestroy 中自动调用）
...
```csharp
// ✅ 推荐：使用 int 类型，性能最佳
public static readonly int EventHpChange = RuntimeId.ToRuntimeId("Player.HpChange");
GameEvent.AddEventListener(EventHpChange, OnHpChange);

// ❌ 不推荐：使用 string 类型，有哈希计算开销
...
```csharp
// ✅ 推荐：在 UI 中使用 AddUIEvent，自动管理生命周期
[Window(UILayer.Top)]
class BattleUI : UIWindow
{
    public override void RegisterEvent()
    {
        ...
        // 无需手动 RemoveEventListener，UI 销毁时自动移除
    }
}
...
{
    public static readonly int OnPlayerHpChange = RuntimeId.ToRuntimeId("Player.HpChange");
    public static readonly int OnPlayerLevelUp = RuntimeId.ToRuntimeId("Player.LevelUp");
    public static readonly int OnBattleStart = RuntimeId.ToRuntimeId("Battle.Start");
    public static readonly int OnBattleEnd = RuntimeId.ToRuntimeId("Battle.End");
}
```

### 4. 使用局部事件管理器

对于需要局部事件管理的场景（如玩家对象），可以使用 `GameEventMgr`：
...
1. **使用 int 类型事件 ID**：避免字符串哈希计算
2. **避免频繁注册/移除事件**：在对象生命周期内保持事件监听
3. **使用 UI 事件自动管理**：减少手动管理带来的性能开销
4. **合理使用事件参数**：避免传递过大的对象，考虑传递 ID 或引用

---

## 🔗 相关链接

- [UI 模块文档](./3-5-UI模块.md) - 了解 UI 模块如何与事件模块配合
- [框架概览](./2-框架概览.md) - 了解 MVE 架构设计
